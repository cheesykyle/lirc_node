...
IRSend.prototype.send_ccf_once = function(code, callback) {
  var command = this._send_ccf_once(code);
  return exec(command, callback);
};
...
IRSend.prototype._send_ccf_once = function(code) {
  if (!code) code = '';
  
  return this.command + ' SEND_CCF_ONCE "' + code + '"';
};
